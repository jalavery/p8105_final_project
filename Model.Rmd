...
ggplot(data = tidy_data_acs, aes(x = mental_health)) +
  geom_histogram() +
  labs(title = "Distribution of the Percent of Adults at the Census-Tract Level with Poor Mental Health",
...
broom::tidy(mdl_mental_heatlh) %>% 
  mutate(term = case_when(term == "physical_inactivity" ~ "% Without physicial activity",
                          ...
                          TRUE ~ term)) %>% 
  knitr::kable(digits = c(2, 2, 2, 2, 2), format.args	= list(p.value = format.pval(digits = 3)))
...
# plot diagnostics
ggplot(data = diagnostic_mh, aes(x = pct_male, y = resid)) + 
  geom_point()

# look at unadjusted associations
ggplot(data = tidy_data, aes(x = health_insurance, y = mental_health)) +
  geom_point() +
  geom_smooth()

ggplot(data = tidy_data, aes(x = current_smoking, y = mental_health)) +
  geom_point() +
  geom_smooth()

#are unhealthy behaviors correlated with each other?
ggplot(data = tidy_data, aes(x = current_smoking, y = binge_drinking)) +
  geom_point() +
  geom_smooth()
```
