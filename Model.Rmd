...
extremes_mh <- bind_rows(worst_mh, best_mh)
...
extremes_mh %>% 
  ...
  coord_flip() +
  labs(title = "Cities with the top 10 best and worst mental health in the US",
       x = "",
...
# mdl_mental_heatlh_random <- lme4::lmer(mental_health ~ physical_inactivity + current_smoking + health_insurance + pct_male + pct_ltHS + pct_white + (1 | city_name), data = tidy_data_acs)
...
## Hypothesis testing (do we want to bootstrap?)
...
bootstrap_models <- bootstrap_samples %>% 
  mutate(models = map(strap, ~lm(mental_health ~ physical_inactivity + current_smoking + health_insurance + pct_male + pct_ltHS + pct_white, data = .x)),
